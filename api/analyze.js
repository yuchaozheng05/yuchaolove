...
- 给 3 条最贴合上下文的 sticker_suggestions。每条包含适合当前上下文的短配字、情绪和画面场景，用作聊天表情包。对方身体不舒服时优先安慰、休息、倾听、拍拍；对方为考试或作业焦虑时优先学习加油；对方开心时优先开心庆祝。不要推荐手机画面。配字要像聊天梗图，不要写分析标签。页面会根据聊天情绪补足到 6 条。
...
候选必须是“我”准备发给“对方”的话，输出 3 到 5 条自然、简短、可以直接发送的回复。最多一条带问号。`;
    ...
    sticker_suggestions: {
      ...
      items: {
        ...
        properties: {
          ...
          scene: { type: 'string', enum: ['comfort', 'rest', 'study', 'listen', 'happy', 'cheer', 'peek', 'confused', 'pat'] },
        },
...
function mergeRefinedReplies(advice, rawText) {
  ...
  const replies = Array.isArray(value.replies)
    ? value.replies
        .map((reply) => ({ text: cleanText(reply?.text, 80) }))
...
function buildDefaultStickerSuggestions(stage) {
  const suggestions = {
    初次认识: [
      { text: '哈哈有点意思', mood: 'playful', scene: 'happy' },
      { text: '展开说说', mood: 'curious', scene: 'peek' },
      { text: '我先听着', mood: 'curious', scene: 'listen' },
      { text: '让我想想', mood: 'curious', scene: 'confused' },
      { text: '收到收到', mood: 'playful', scene: 'cheer' },
      { text: '继续继续', mood: 'playful', scene: 'pat' },
    ],
    轻松破冰: [
      { text: '行 你继续', mood: 'teasing', scene: 'happy' },
      { text: '真的假的', mood: 'playful', scene: 'confused' },
      { text: '我再看看', mood: 'curious', scene: 'peek' },
      { text: '有点意思', mood: 'playful', scene: 'cheer' },
      { text: '让我听听', mood: 'caring', scene: 'listen' },
      { text: '什么情况', mood: 'speechless', scene: 'comfort' },
    ],
    稳定了解: [
      ...
      { text: '慢慢说', mood: 'caring', scene: 'comfort' },
      { text: '这么回事', mood: 'playful', scene: 'happy' },
    ],
    暧昧升温: [
      { text: '有点会聊', mood: 'teasing', scene: 'peek' },
      { text: '我再观察', mood: 'playful', scene: 'confused' },
      { text: '行吧 加一分', mood: 'teasing', scene: 'cheer' },
      { text: '被你拿捏了', mood: 'playful', scene: 'happy' },
      { text: '有点可爱', mood: 'caring', scene: 'pat' },
      { text: '先别得意', mood: 'teasing', scene: 'comfort' },
    ],
    情绪陪伴: [
      ...
      { text: '先别着急', mood: 'caring', scene: 'peek' },
    ],
    建议停手: [
      { text: '行 你继续玩', mood: 'retreat', scene: 'rest' },
      { text: '那我先撤了', mood: 'retreat', scene: 'peek' },
      { text: '所以我算什么', mood: 'speechless', scene: 'confused' },
      { text: '好吧好吧', mood: 'retreat', scene: 'listen' },
      { text: '我先消失', mood: 'retreat', scene: 'comfort' },
      { text: '当我没说', mood: 'speechless', scene: 'pat' },
    ],
  };
...
function buildContextualStickerSuggestions(context, stage = '轻松破冰') {
  const contextualSuggestions = {
    physical_discomfort: [
      ...
      { text: '作业先放放', mood: 'caring', scene: 'study' },
    ],
    study_stress: [
    ...
    happy: [
      ...
      { text: '可以可以', mood: 'playful', scene: 'pat' },
      { text: '有点可爱', mood: 'teasing', scene: 'peek' },
      { text: '收到快乐', mood: 'playful', scene: 'listen' },
      { text: '继续保持', mood: 'playful', scene: 'study' },
    ],
