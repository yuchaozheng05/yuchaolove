...
- 给 3 条最贴合上下文的 sticker_suggestions。每条包含适合当前上下文的短配字、情绪和画面场景，用作聊天表情包。配字要像聊天梗图，不要写分析标签。页面会根据聊天阶段补足到 6 条。
...
function parseAdvice(rawText) {
  ...
  return {
    ...
    interest_score: isChatScreenshot ? (emotionalDisclosure ? Math.max(52, clampScore(value.interest_score)) : activeCuriosity ? Math.max(62, clampScore(value.interest_score)) : clampScore(value.interest_score)) : 0,
